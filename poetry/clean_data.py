...
def clean_cn_corpus(file_name, save_to_file=None, clean_level='all', simple_only=True, is_save=True):
    """
    ...
    if not save_to_file:
        save_to_file = os.path.join(base_dir, os.path.basename(file_name).split('.')[0] + '_cleaned.txt')
    ...
    if is_save:
        with open(save_to_file, 'w+', encoding='utf-8') as f:
            for l in clean_content:
                f.write(l + '\n')
        print('[INFO] cleaned file have been saved to %s.' % save_to_file)
